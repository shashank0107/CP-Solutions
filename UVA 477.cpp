...
struct circle{
    point cen;
    double rad;
    circle(point cen=point(1, 2), double rad=0) : cen(cen), rad(rad) {}
};

struct seg{
    point pt[4];
    seg(point a = point(), point b = point(), point c = point(), point d = point()){
        pt[0] = a;
        pt[1] = b;
        pt[2] = c;
        pt[3] = d;
...
struct figure {
    ...
    circle c;
    char ch;
    figure(char ch, point a, point b, point c, point d) : s(a, b, c, d), ch(ch) {}
    figure(char ch, point cp, double r) : c(cp, r), ch(ch) {}
};

bool insideRect(point p, seg s){

    for(int i = 0; i < 4; i++){
        //trace2(i, (ccw(s.pt[i], s.pt[(i+1)%4], p)))
        if ( ccw(s.pt[i], s.pt[(i+1)%4], p) > 0 )  
            return false;
    }
    return true;
}

bool insideCirc(point p, circle c){
    if (length(p-c.cen) > c.rad+EPS)  return false;
    return true;
}
...
void solve(){
    ...
    while(cin >> c){
        ...
        if (c == 'r'){
            ...
            cin >> x1 >> y1 >> x2 >> y2;
            v.pb(figure('r', point(x1, y1), point(x2, y1), point(x2, y2), point(x1, y2)));
        }
        else{
            cin >> x1 >> y1 >> r;
            v.pb(figure('c', point(x1, y1), r));
        }
    }
    int n = v.size(), cnt = 1;
    double x, y;
    ...
    while(cin >> x >> y)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            //trace2(v[i].ch, i);
            if (v[i].ch == 'r'){
                if (insideRect(point(x, y), v[i].s))  ans.pb(i+1);
            }
            else{
                if (insideCirc(point(x, y), v[i].c))  ans.pb(i+1);
            }
        }

        if (ans.empty())    cout << "Point " << cnt << " is not contained in any figure" << endl;
        else
            for(auto i : ans)   cout << "Point " << cnt << " is contained in figure " << i << endl;
        cnt++;
    }
